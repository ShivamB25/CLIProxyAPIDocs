...
После предыдущих практических занятий мы успешно интегрировали `Gemini CLI` и `Codex` в `CLIProxyAPI`. В этом выпуске мы расскажем, как добавить Cookie от Gemini Web, чтобы `CLIProxyAPI` поддерживал модель `NanoBanana`.
